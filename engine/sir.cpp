...
// RK4 integrator for SEIR model
void step_seir(Population& pop, double dt) {
    // Calculate total population (handles mortality)
    double N = pop.S + pop.E + pop.I + pop.R;
    if (N < 1e-10) N = 1.0; // Avoid division by zero
    ...
    // Ensure non-negative and prevent tiny negative values
